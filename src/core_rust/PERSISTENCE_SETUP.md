...
Quick guide для настройки PostgreSQL backend для Axiom OS v0.26.0.
...
CREATE USER axiom_user WITH ENCRYPTED PASSWORD 'your_secure_password';
...
# Опция 1: Через psql
psql -U axiom_user -d axiom_db -f schema.sql
...
```bash
DATABASE_URL=postgres://axiom_user:your_secure_password@localhost/axiom_db
```

Сохрани в `.env` файл:

```bash
echo 'DATABASE_URL=postgres://axiom_user:your_secure_password@localhost/axiom_db' > .env
```
...
```bash
# Daily backup
pg_dump -U axiom_user axiom_db > backup_$(date +%Y%m%d).sql

# Restore
psql -U axiom_user -d axiom_db < backup_20250114.sql
```
...
ALTER DATABASE axiom_db SET log_min_duration_statement = 100;  -- 100ms
...
docker run --name axiom-postgres \
    -e POSTGRES_USER=axiom_user \
    -e POSTGRES_PASSWORD=your_secure_password \
    -e POSTGRES_DB=axiom_db \
    -p 5432:5432 \
    -v axiom_data:/var/lib/postgresql/data \
    -d postgres:15

# Применение схемы
docker exec -i axiom-postgres psql -U axiom_user -d axiom_db < schema.sql
```

---

**Готово!** PostgreSQL backend настроен для Axiom OS v0.26.0 🚀