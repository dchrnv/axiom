...
fn frameweaver_end_to_end_smoke() {
    ...
    engine.ashti.inject_connection(MAYA_ID, syn_conn(10, 20, 1)).unwrap();
    engine.ashti.inject_connection(MAYA_ID, syn_conn(10, 30, 2)).unwrap();
    ...
    let exp_idx = engine.ashti.index_of(EXPERIENCE_ID).expect("EXPERIENCE domain must exist");
    let exp_state = engine.ashti.state(exp_idx).expect("EXPERIENCE state must be accessible");

    let frame_anchors: Vec<_> = exp_state.tokens.iter()
    ...
    let exp_bonds: Vec<_> = exp_state.connections.iter()
    ...
    for &target_sutra_id in &[10u32, 20u32, 30u32] {
        if engine.ashti.find_token_by_sutra_id(EXPERIENCE_ID, target_sutra_id).is_none() {
            engine.ashti.inject_token(
                EXPERIENCE_ID,
    ...
    let unfold_cmd = UclCommand::new(OpCode::UnfoldFrame, 0, 10, 0)
        .with_payload(&unfold_payload);
    let unfold_result = engine.process_command(&unfold_cmd);

    assert_eq!(
        unfold_result.status, axiom_ucl::CommandStatus::Success as u8,
        "UnfoldFrame должен завершиться успешно"
    );

    // Шаг 6: Проверить: в LOGIC появился токен-анкер и связи
    assert_eq!(
        engine.ashti.token_count(LOGIC_ID), logic_tokens_before + 1,
        "В LOGIC должен появиться один новый токен (копия Frame-анкера)"
    );
    let logic_conns_after = engine.ashti.state(engine.ashti.index_of(LOGIC_ID).unwrap()).unwrap().connections.len();
    assert!(
    ...
    assert_eq!(engine.frame_weaver.stats.scans_performed, 25, "должно быть 25 сканов");
    assert!(engine.frame_weaver.stats.candidates_detected >= 1, "хотя бы 1 кандидат обнаружен");
    assert!(engine.frame_weaver.stats.crystallizations_approved >= 1, "хотя бы 1 кристаллизация");
    assert!(engine.frame_weaver.stats.frames_in_experience >= 1, "Frame виден в stats");
    assert_eq!(engine.frame_weaver.stats.unfold_requests, 1, "1 UnfoldFrame запрос");

    // Шаг 8: Оригинальный Frame в EXPERIENCE остался нетронутым
    assert!(
        engine.ashti.find_token_by_sutra_id(EXPERIENCE_ID, anchor_id).is_some(),
