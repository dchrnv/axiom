"""
Rich display formatters for Axiom objects in Jupyter.
"""
...
def format_query_result_html(result: Any) -> str:
    ...
    if hasattr(result, "nodes") and result.nodes:
        html_parts.append('<div class="axiom-result">')
        html_parts.append(
            f'<div class="axiom-header">🔵 Nodes: '
            f'<span class="axiom-count">{len(result.nodes)}</span></div>'
        )
        html_parts.append('<table class="axiom-table">')
        html_parts.append(
        ...
        for node in result.nodes[:50]:  # Limit to 50 for display
            ...
            html_parts.append(
                f'<tr>'
                f'<td class="axiom-id">{node_id}</td>'
                f'<td>{node_type}</td>'
                f'<td class="axiom-props">{props_str}</td>'
                f'</tr>'
            )
    ...
    if hasattr(result, "edges") and result.edges:
        html_parts.append('<div class="axiom-result">')
        html_parts.append(
            f'<div class="axiom-header">🔗 Edges: '
            f'<span class="axiom-count">{len(result.edges)}</span></div>'
        )
        html_parts.append('<table class="axiom-table">')
        html_parts.append(
        ...
        for edge in result.edges[:50]:  # Limit to 50 for display
            ...
            html_parts.append(
                f'<tr>'
                f'<td class="axiom-id">{source}</td>'
                f'<td class="axiom-id">{target}</td>'
                f'<td>{edge_type}</td>'
                f'<td class="axiom-props">{props_str}</td>'
                f'</tr>'
            )
    ...
    if (not hasattr(result, "nodes") or not result.nodes) and \
       (not hasattr(result, "edges") or not result.edges):
        html_parts.append('<div class="axiom-result">')
        html_parts.append(
            '<div class="axiom-header">📭 Empty Result</div>'
        )
        html_parts.append(
...
def install_display_formatters(ipython):
    """
    Install rich display formatters for Axiom objects.

    This enables automatic rich HTML display when Axiom objects
    ...
    def axiom_aware_format(obj):
        # Try Axiom formatting first
