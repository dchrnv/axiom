...
После `just run-grafana` всё доступно по этим адресам:

| Адрес | Что |
|-------|-----|
| `http://localhost:8080` | Workstation V2 — основной UI |
| `http://localhost:8080/metrics` | Prometheus-метрики движка |
| `ws://localhost:8080/api/ws` | WebSocket JSON поток (снапшоты + события) |
| `ws://localhost:9876` | BroadcastServer (бинарный протокол, Workstation V2) |
| `http://localhost:3000` | Grafana — дашборды в реальном времени |
| `http://localhost:9090` | Prometheus — raw метрики и запросы |
| `http://localhost:5173` | Workstation V2 dev-режим (только `just dev`) |

> **Минимальный старт** без Grafana: `just run` → только `:8080` и `:9876`.
