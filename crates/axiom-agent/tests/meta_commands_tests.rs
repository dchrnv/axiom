...
use axiom_agent::meta_commands::{handle_meta_read, handle_meta_mutate, MetaAction};
...
fn make_perf()   -> PerfTracker { PerfTracker::new_for_test() }
...
fn read(line: &str) -> String {
    ...
    handle_meta_read(
        line, &engine, None, &config,
        &HashSet::new(), &VecDeque::new(), &make_perf(), 0, 0,
    )
}
...
fn test_handle_meta_read_status_nonempty() {
    let out = read(":status");
    assert!(out.contains("tick_count"), "expected tick_count in :status output");
}
...
fn test_handle_meta_mutate_save_creates_file() {
    ...
    assert!(result.output.contains("saved"), "expected 'saved' in output");
...
fn test_handle_meta_mutate_load_replaces_engine() {
    ...
    let mut saver  = make_saver();
    let config = CliConfig::default();
    handle_meta_mutate(&format!(":save {}", dir_str), &mut engine, &mut saver, &config);

    // Теперь загружаем
    let result = handle_meta_mutate(&format!(":load {}", dir_str), &mut engine, &mut saver, &config);
    assert!(matches!(result.action, MetaAction::EngineReplaced), "expected EngineReplaced");
}
