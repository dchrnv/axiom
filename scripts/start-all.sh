...
echo "🚀 Starting Axiom Full Stack..."
echo ""
...
if command -v tmux &> /dev/null; then
    ...
    tmux kill-session -t axiom 2>/dev/null || true

    # Create new session
    tmux new-session -d -s axiom -n backend
    tmux send-keys -t axiom:backend "cd $PROJECT_ROOT && ./scripts/start-backend.sh" C-m

    # Create window for frontend
    tmux new-window -t axiom -n frontend
    tmux send-keys -t axiom:frontend "cd $PROJECT_ROOT && ./scripts/start-frontend.sh" C-m

    echo ""
    echo "✅ Services started in tmux session 'axiom'"
    echo ""
    echo "📋 Available commands:"
    echo "   tmux attach -t axiom        - Attach to session"
    echo "   tmux kill-session -t axiom  - Stop all services"
    echo ""
    echo "🌐 Access points:"
