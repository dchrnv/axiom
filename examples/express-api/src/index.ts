/**
 * Express.js API example with Axiom integration.
 *
 * This example shows how to build a REST API that proxies
 * Axiom functionality with additional business logic.
 */
...
app.get('/health', async (req: Request, res: Response) => {
  try {
    const health = await axiom.health.check();
    res.json({ status: 'ok', axiom: health });
  } catch (error) {
...
app.post('/documents', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const token = await retryWithBackoff(
      () => axiom.tokens.create({ text, metadata }),
      { maxRetries: 3 }
    );
...
app.get('/documents/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const token = await axiom.tokens.get(id);
...
app.get('/documents', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const tokens = await axiom.tokens.list({ limit, offset });
...
app.post('/documents/search', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const results = await axiom.tokens.queryByText({
      text: query,
      topK: top_k || 10,
...
app.put('/documents/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const updated = await axiom.tokens.update(id, { text, metadata });
...
app.listen(PORT, () => {
  ...
  console.log(`Axiom API: ${process.env.NEUROGRAPH_API_URL || 'http://localhost:8000'}`);
});
