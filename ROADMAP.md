...
**Версия:** 7.0
**Дата:** 2026-03-20
**Статус:** v0.6.2 завершена, планирование v0.7.0 и v0.8.0
...
- `runtime/src/space.rs` - новый модуль
...
**1.2 Spatial Hash Grid операции**
...
- Линейная модель (первая версия)
- Inverse_square модель (опционально)
- Конфигурируемый `gravity_scale_shift`
- Тесты: разные массы, расстояния, модели

**1.5 Инерция и движение**
...
- Тесты: движение, трение, переполнение

**1.6 Пространственные события**
- Расширить `EventType`:
  - `TokenMoved` (0x0010)
  - `TokenCollision` (0x0011)
  - `TokenEnteredCell` (0x0012)
- Генерация событий при движении
- Обнаружение столкновений через spatial hash
- Тесты: генерация событий, collision detection

**1.7 Интеграция с Domain**
- Добавить `SpatialHashGrid` в `DomainState`
- Перестройка индекса после batch событий
- Конфигурация `rebuild_frequency`
- Тесты: domain + spatial hash
...
- Проверка гравитации/инерции для всех токенов
- Тесты: heartbeat → space checks
...
### Оценка:
- SPACE V6.0: ~8-10 шагов (1.1-1.11)
