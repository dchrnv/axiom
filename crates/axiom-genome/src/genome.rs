...
impl std::fmt::Display for GenomeError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            GenomeError::InvalidConfig(field) => write!(f, "Invalid genome config: {field}"),
...
impl Genome {
    ...
    pub fn default_ashti_core() -> Self {
        ...
        let protocol_rules = vec![
            ...
            ProtocolRule {
                source: M::Sutra,
                target: M::Experience,
                data_type: D::TokenReference,
                mandatory: true,
            },
            ProtocolRule {
                source: M::Experience,
                target: M::Arbiter,
                data_type: D::ResonanceResponse,
                mandatory: true,
            },
            ProtocolRule {
                source: M::Arbiter,
            ...
            ProtocolRule {
                source: M::Arbiter,
                target: M::Logic,
                data_type: D::PatternHint,
                mandatory: true,
            },
            ProtocolRule {
                source: M::Logic,
                target: M::Maya,
                data_type: D::ProcessingResult,
                mandatory: true,
            },
            ProtocolRule {
                source: M::Logic,
                target: M::Experience,
                data_type: D::NewExperience,
                mandatory: true,
            },
            ProtocolRule {
                source: M::Maya,
                target: M::Arbiter,
                data_type: D::ComparisonResult,
                mandatory: false,
            },
            ProtocolRule {
                source: M::Arbiter,
                target: M::Experience,
                data_type: D::Feedback,
                mandatory: false,
            },
        ];
    ...
    pub fn validate(&self) -> Result<(), GenomeError> {
        ...
        if self.invariants.connection_size != 64 {
            return Err(GenomeError::InvariantViolation(
                "connection_size must be 64",
            ));
        }
        ...
        if self.invariants.domain_config_size != 128 {
            return Err(GenomeError::InvariantViolation(
                "domain_config_size must be 128",
            ));
        }
        ...
        if !self.invariants.no_wall_clock_in_core {
            return Err(GenomeError::InvariantViolation(
                "no_wall_clock_in_core must be true",
            ));
        }
        if !self.invariants.event_id_monotonic {
            return Err(GenomeError::InvariantViolation(
                "event_id_monotonic must be true",
            ));
        }
        ...
        for (src, tgt, name) in mandatory {
            let found = self
                .protocol_rules
                .iter()
                .any(|r| r.source == src && r.target == tgt && r.mandatory);
            if !found {
        ...
        if self.config.default_heartbeat_interval == 0 {
            return Err(GenomeError::InvalidConfig(
                "default_heartbeat_interval must be > 0",
            ));
        }
