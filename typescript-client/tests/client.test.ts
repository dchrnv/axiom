...
import { AxiomClient, NotFoundError, AuthenticationError } from '../src';
import type { Token } from '../src';

describe('AxiomClient', () => {
  let client: AxiomClient;
  let createdTokens: Token[] = [];

  beforeAll(() => {
    client = new AxiomClient({
  ...
  describe('Authentication', () => {
    it('should throw AuthenticationError with wrong credentials', async () => {
      const badClient = new AxiomClient({
        baseUrl: 'http://localhost:8000',
        username: 'wrong',
        password: 'wrong',
      });
