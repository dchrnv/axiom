...
OBS пишет `training_data.jsonl` каждые 200 тиков: one-hot кольца (short/mid/long × 9 подсистем) + teacher reactivation_weights[8].
...
**Блокер:** сначала закрыть NEURAL-TD-02 (мисматч формата).

**Когда:** после накопления ≥10K примеров — то есть после первого production OBS-прогона с реальным текстом (CORPUS-TD-01).

---

### NEURAL-TD-02 — Мисматч формата: training_data vs INPUT_SIZE модели

**Где:** `crates/axiom-observe/src/training.rs` + `crates/axiom-neural/src/reactivation_depth.rs` + `crates/axiom-runtime/src/over_domain/neural_advisor/neural_depth.rs`

Текущий конфликт:
- `training.rs` пишет one-hot кольца: 9×16 + 9×64 + 9×256 = **3024 признака**
- `reactivation_depth.rs` ожидает FFT: `INPUT_SIZE = 9 × 171 = **1539**`
- `neural_depth.rs` mode=Neural: подаёт `extract_onehot_rings()` → 3024 → `ShapeMismatch` → silent fallback

Нужно выбрать один формат и сделать конвейер консистентным:
- Вариант A — FFT везде: `training.rs` прогоняет кольца через `ActivityFft` перед сохранением
- Вариант B — one-hot везде: `INPUT_SIZE = 3024`, убрать FFT из `reactivation_depth`

**Когда:** до тренировки; блокирует NEURAL-TD-01. Небольшая работа (~1 час) как только выбран вариант.
