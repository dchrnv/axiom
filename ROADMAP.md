...
`config/anchors/writing/primitives.yaml` (7 графических примитивов) и
`config/anchors/mathematics/primitives.yaml` (7 структурных примитивов) готовы.
ContextRecognizer подхватывает их через `get_subsystem("writing")` / `get_subsystem("mathematics")`.

Слоевые якоря L1–L4, L6–L8 — отдельная задача Phase E (E1, без дедлайна).
