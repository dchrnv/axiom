# Axiom OS - Multi-stage Docker Build
# Target: API Server (axiom-api binary)
...
RUN cargo build --release --bin axiom-api
...
WORKDIR /build/src/core_rust
RUN cargo build --release --bin axiom-api && \
    strip target/release/axiom-api
...
RUN addgroup -g 1000 axiom && \
    adduser -D -u 1000 -G axiom axiom
...
COPY --from=builder /build/src/core_rust/target/release/axiom-api /app/axiom-api

# Set ownership
RUN chown axiom:axiom /app/axiom-api
...
ENV AXIOM_PORT=8080
ENV AXIOM_MAX_TOKENS=10000000
ENV AXIOM_MAX_MEMORY_BYTES=1073741824
