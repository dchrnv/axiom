...
В структуру `FrameCandidate` добавлено поле `shell_similarity: f32` — среднее попарное cosine-сходство shell-профилей (`[u8; 8]`) участников кандидата.

- Вычисляется при построении кандидата в `scan_state`
- Высокое значение (~1.0): участники семантически однородны (один слой, одна тема)
- Низкое значение (~0.0): участники из разных слоёв (хаотичный паттерн)

Метод `FrameWeaver::avg_candidate_shell_similarity() -> f32` возвращает среднее по всем активным кандидатам. Доступен для диагностики и метрик.
