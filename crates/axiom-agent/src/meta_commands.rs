...
use axiom_persist::{
    export_skills, export_traces, import_skills, import_traces, load as persist_load,
    save as persist_save, AutoSaver, WriteOptions,
};
use axiom_runtime::AxiomEngine;
use axiom_ucl::{OpCode, UclCommand};

use crate::channels::cli::{fmt_ns, CliConfig, CliConfigFile, PerfTracker};
    ...
    match parts[0] {
        ...
        ":traces" => {
            ...
            if traces.is_empty() {
                ...
                writeln!(
                    out,
                    "  Total: {}  |  Avg weight: {:.2}  |  Max weight: {:.2}",
                    traces.len(),
                    avg_w,
                    max_w
                )
        ...
        ":arbiter" => {
            ...
            writeln!(
                out,
                "  reflex success:    {}  fail: {}",
                reflector.total_success(),
                reflector.total_fail()
            )
            .unwrap();
        }
        ...
        ":perf" => {
            ...
            if s.adaptation_interval > 0 {
                writeln!(
                    out,
                    "  adaptation:   {} calls (every {} ticks)",
                    t / s.adaptation_interval as u64,
                    s.adaptation_interval
                )
                .unwrap();
            }
            if s.horizon_gc_interval > 0 {
                writeln!(
                    out,
                    "  horizon_gc:   {} calls (every {} ticks)",
                    t / s.horizon_gc_interval as u64,
                    s.horizon_gc_interval
                )
                .unwrap();
            }
            if s.dream_interval > 0 {
                writeln!(
                    out,
                    "  dream:        {} calls (every {} ticks)",
                    t / s.dream_interval as u64,
                    s.dream_interval
                )
                .unwrap();
            }
            if s.tension_check_interval > 0 {
                writeln!(
                    out,
                    "  tension_chk:  {} calls (every {} ticks)",
                    t / s.tension_check_interval as u64,
                    s.tension_check_interval
                )
                .unwrap();
            }
        }
        ...
        ":trace" => match parts.get(1).and_then(|s| s.parse::<usize>().ok()) {
            None => writeln!(
                out,
                "  Usage: :trace <index>  (1-based, same as :traces output)"
            )
            .unwrap(),
            Some(0) => writeln!(out, "  index must be ≥ 1").unwrap(),
            Some(idx) => {
                let exp = engine.ashti.experience();
                let traces = exp.traces();
                let mut sorted: Vec<_> = traces.iter().enumerate().collect();
                ...
                match sorted.get(idx - 1) {
                    None => writeln!(out, "  trace #{} not found (total: {})", idx, traces.len())
                        .unwrap(),
                    Some((_, t)) => {
                        let tick = engine.tick_count;
                        let age = tick.saturating_sub(t.created_at);
                        ...
                        writeln!(
                            out,
                            "  velocity:      ({}, {}, {})",
                            t.pattern.velocity[0], t.pattern.velocity[1], t.pattern.velocity[2]
                        )
                        .unwrap();
        ...
        ":connections" => {
            ...
            for (id, state) in &states {
                ...
                for (i, c) in state.connections.iter().take(10).enumerate() {
                    writeln!(
                        out,
                        "  {:>3}  {:>8}→{:<8}  strength={:.2}  stress={:.2}  type={:#06x}",
                        i + 1,
                        c.source_id,
                        c.target_id,
                        c.strength,
        ...
        ":dream" => {
            ...
            writeln!(
                out,
                "  crystallizable:   {} (weight≥0.9, success≥5)",
                candidates.len()
            )
            .unwrap();
        ...
        ":dream-stats" => {
            ...
            writeln!(
                out,
                "  By trigger:  Idle={}, Fatigue={}, Explicit={}",
                sched.idle_triggers, sched.fatigue_triggers, sched.explicit_triggers
            )
            .unwrap();
            let cycle = &engine.dream_cycle.stats;
            writeln!(
                out,
                "  Cycles:  total={}, complete={}, timeout={}, approved={}, vetoed={}",
                cycle.total_cycles,
                cycle.completed_cycles,
                cycle.timed_out_cycles,
                cycle.total_approved,
                cycle.total_vetoed
            )
            .unwrap();
        }
        ...
        ":impulses" => {
            ...
            writeln!(
                out,
                "  tension traces:  {} (each may generate impulse)",
                tension_n
            )
            .unwrap();
        ...
        ":unwatch" => match parts.get(1).copied() {
        ...
        ":schema" => {
            ...
            match kind {
                ...
                "domain" => {
                    writeln!(out, "{}", axiom_config::schema::domain_schema_json()).unwrap()
                }
                "heartbeat" => {
                    writeln!(out, "{}", axiom_config::schema::heartbeat_schema_json()).unwrap()
                }
                ...
                _ => {
                    ...
                    writeln!(
                        out,
                        "    axiom     — JSON-схема axiom.yaml (корневой конфиг)"
                    )
                    .unwrap();
                    writeln!(
                        out,
                        "    domain    — JSON-схема доменного конфига (presets/domains/)"
                    )
                    .unwrap();
        ...
        ":anchors" => match anchor_set {
            None => writeln!(
                out,
                "  no anchors loaded (config/anchors/ not found or empty)"
            )
            .unwrap(),
            Some(set) => {
                ...
                match sub {
                    None => {
                        ...
                        writeln!(
                            ...
                            (1..=8)
                                .filter(|&i| !set.layers[i - 1].is_empty())
                                .map(|i| format!("L{}", i))
                                .collect::<Vec<_>>()
                                .join(", ")
                        )
                        ...
                        writeln!(
                            ...
                            (1..=8)
                                .filter(|&i| !set.domains[i - 1].is_empty())
                                .map(|i| format!("D{}", i))
                                .collect::<Vec<_>>()
                                .join(", ")
                        )
                    ...
                    Some("layer") => {
                        ...
                        if idx == 0 || idx > 8 {
                            ...
                            for a in anchors {
                                let [x, y, z] = a.position;
                                writeln!(out, "  {:20}  pos=({:7},{:7},{:7})", a.word, x, y, z)
                                    .unwrap();
                            }
                        }
                    }
        ...
        ":match" => {
            ...
            if text.is_empty() {
                ...
                match anchor_set {
                    None => writeln!(out, "  no anchors loaded — would use FNV-1a hash fallback")
                        .unwrap(),
                    Some(set) => {
        ...
        ":domain" => match parts.get(1).and_then(|s| s.parse::<u16>().ok()) {
            ...
            Some(id) => match engine.ashti.config_of(id) {
                ...
                Some(cfg) => {
                    ...
                    writeln!(
                        out,
                        "  token_capacity:   {} (used: {})",
                        cfg.token_capacity, tokens
                    )
                    .unwrap();
                    writeln!(
                        out,
                        "  connection_cap:   {} (used: {})",
                        cfg.connection_capacity, conns
                    )
                    .unwrap();
                    ...
                    writeln!(
                        out,
                        "  min_coherence:    {:.2}",
                        cfg.min_coherence as f32 / 255.0
                    )
                    .unwrap();
    ...
    let action =
        match parts[0] {
            ...
            ":wake-up" => {
                ...
                writeln!(
                    output,
                    "  Critical wake signal sent. State: {:?}",
            ...
            ":autosave" => match parts.get(1).copied() {
                Some("on") => {
                    let interval = parts.get(2).and_then(|s| s.parse().ok()).unwrap_or(1000u32);
            ...
            ":export" => {
                ...
                match what {
                    "skills" => match export_skills(engine, path) {
                        Ok(r) => writeln!(output, "  exported {} skills → {}", r.exported, r.path)
                            .unwrap(),
                        Err(e) => writeln!(output, "  export failed: {e}").unwrap(),
                    },
                    "traces" => match export_traces(engine, path, 0.0) {
                        Ok(r) => writeln!(output, "  exported {} traces → {}", r.exported, r.path)
                            .unwrap(),
                        Err(e) => writeln!(output, "  export failed: {e}").unwrap(),
                    },
