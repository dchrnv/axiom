...
use axiom_space::{SpatialHashGrid, distance2};
...
fn bench_grid_rebuild(c: &mut Criterion) {
    ...
    for count in [100, 500, 1000, 5000] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(count), &positions, |b, positions| {
            b.iter(|| {
...
fn bench_distance2(c: &mut Criterion) {
    c.bench_function("distance2", |b| {
        b.iter(|| black_box(distance2(
            black_box(100i16), black_box(200i16), black_box(300i16),
            black_box(400i16), black_box(500i16), black_box(600i16),
        )))
    });
}
