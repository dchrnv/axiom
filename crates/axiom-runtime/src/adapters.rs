...
///
/// Реализуется для каждого транспорта (stdin, Telegram, WebSocket и т.д.).
/// Ядро не знает о транспорте — только о команде.
pub trait Perceptor: Send {
    /// Получить следующую команду (неблокирующий или блокирующий — на усмотрение impl).
    ///
    /// Возвращает `None` если команд нет или адаптер сигнализирует о завершении.
    fn receive(&mut self) -> Option<UclCommand>;
    /// Имя адаптера для логирования и конфигурации
    fn name(&self) -> &str;
}

/// Исходящий адаптер: Event / UclResult → внешний мир.
///
/// Реализуется для каждого транспорта (stdout, Telegram, shell и т.д.).
pub trait Effector: Send {
    /// Отправить событие во внешний мир
    fn emit(&mut self, event: &Event);
    /// Отправить результат команды во внешний мир
