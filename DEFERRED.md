...
### CR-TD-01 — sync_axial_store требует внешнего координатора

**Где:** `crates/axiom-runtime/src/over_domain/context_recognizer/mod.rs`

`ContextRecognizer::sync_axial_store(&AxialStore)` должен вызываться координатором рантайма
после каждого цикла `AxialEvaluator`. Пока механизма межкомпонентной коммуникации внутри
`on_tick` нет — компоненты получают только `&AshtiCore`.

Если `sync_axial_store` не вызывается — снапшот пустой, `current_active_octants_for`
возвращает `[]`, `ScanningPlan` падает в поверхностный fallback `CreativeAffirmation`.
Система работает, но без октантного контекста от AxialEvaluator.

**Варианты V2:** event/bus между over-domain компонентами; или `Arc<RwLock<AxialStore>>`
в shared state рантайма.

**Спека:** ContextRecognizer_V5_0.md §12 CR-TD-01.
...
### NA-TD-01 — sync_*_store требует внешнего координатора

**Где:** `crates/axiom-runtime/src/over_domain/neural_advisor/mod.rs`

`NeuralAdvisor::sync_axial_store()`, `sync_profile_store()`, `sync_depth_store()` должны вызываться
координатором рантайма после соответствующих тиков AxialEvaluator и ContextRecognizer.
Пока координатора нет — NeuralAdvisor работает с пустыми снапшотами и видит только Frame-анкеры
через AshtiCore. Советники корректно возвращают None/Unresolved без данных.

**V2:** ввести координатор (отдельный компонент или расширение AxiomEngine), который
после каждого тика AxialEvaluator вызывает `na.sync_axial_store(ae.axial_store())`,
и после каждого тика ContextRecognizer — `na.sync_profile_store(cr.profile_store())` и
`na.sync_depth_store(cr.depth_store())`.

**Спека:** NeuralAdvisor_V1_0.md §11 (sync pattern, CR-TD-01 аналог).
...
### NA-TD-03 — ApproveEmergentCandidate не обрабатывается

**Где:** `crates/axiom-runtime/src/engine.rs`

UCL OpCode `ApproveEmergentCandidate=5201` определён в axiom-ucl, но handler в Engine
не реализован. Команда от оператора (chrnv → Workstation → Engine) будет проигнорирована.

**Что нужно:** добавить в `engine.rs` ветку `OpCode::ApproveEmergentCandidate` →
`neural_advisor.approve_emergent(payload.sutra_id)`.

**Когда:** при подключении NeuralAdvisor к Workstation (NA-TD-02).
