...
## Seed Injection C1 — Live Test (2026-06-14)

**Файл:** `docs/archive/Стихи 2025.md` — 14 стихотворений (2025–2026), серия Таро (00–14) и другие.  
**Тест:** `cargo test -p axiom-agent --test ingest_poetry_test -- --ignored`  
**Параметры:** 200 раундов × 146 чанков × (492 основных + 5197 C1 биграмм) команд = **1.137M UCL команд**

### Параметры инъекции

| Уровень | Что | Кол-во/раунд | mass | semantic_weight |
|---------|-----|-------------|------|----------------|
| C0 (anchor) | 107 графем в crystal (фиксированные) | — | — | позиционный справочник |
| текст (grow) | чанки стихов → TextPerceptor Path 1/3/4 | 146 inject | 50–250 | 0.75–1.0 |
| C1 (grow) | биграммы из crystal C0 | **5 197** inject | 120 | 0.65 |

### Рост Experience traces

| Раунд | Traces | Δ | Matched |
|-------|--------|---|---------|
| 1 | 831 | +831 | 816 |
| 5 | 1000 | +169 | 986 |
| 10 | 1000 | = | 989 |
...
*Насыщение к раунду 5 (лимит буфера Experience). К раунду 25 — вытеснение слабых паттернов, стабилизация на 971.*

### Финальное состояние (раунд 200, 4000 тиков)

| Метрика | Значение |
|---------|---------|
| Experience traces | **971** |
| Matched (узнано) | **958 / 971 = 98.7%** |
| Tension traces | 0 |
| Crystallized frames | 0 *(кристаллизация требует длинных DREAM-циклов)* |

### Vs без C1 (предыдущий прогон)

| | Без C1 | С C1 | Рост |
|-|--------|------|------|
| Команд/раунд | 492 | 5 689 | +11.6× |
| C1 биграмм | 0 | 5 197 | — |
| Experience traces | 116 | **971** | **+8.4×** |
| Matched | 115 | **958** | **+8.3×** |
| Match rate | 99.1% | **98.7%** | ≈ |

### Детекция подсистем (dry-run по чанкам)
...
*writing=73 ожидаемо (поэзия). mathematics=39 — структурность мышления, «while(true)», геометрия образов.*
...
| **C1 Seed Injection** | **971 traces · 98.7% match · 8.4× рост vs C0 only** |
