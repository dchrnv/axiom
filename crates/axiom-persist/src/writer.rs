...
pub fn save(
    ...
    dir: &Path,
    opts: &WriteOptions,
) -> Result<MemoryManifest, PersistError> {
    ...
    let domains: Vec<StoredDomain> = snapshot
        .domains
        .iter()
        .map(|ds| StoredDomain {
    ...
    let traces: Vec<StoredTrace> = experience
        .traces()
        .iter()
    ...
    // Tension traces
    let tension: Vec<StoredTensionTrace> = experience
        .tension_traces()
        .iter()
