...
Этот документ описывает **четыре версии** ContextRecognizer (V6-V9) с включением соответствующих эволюций NeuralAdvisor. V6 — историческая запись, V7 — частично done, V8/V9 — активный план.

---

## 1. V6 — Meta-level Recognition ✅ ЗАВЕРШЕНО
...
### 1.7 SyntacticBridge ✅ РЕАЛИЗОВАНО
...
**Место реализации:** `axiom-runtime/src/orchestrator.rs` — функция `bridge_to_maya()`. ✅ Реализовано.

---

### 1.8 Cross-modal preparation (нюанс) ⏳ ОТЛОЖЕНО
...
Сейчас всегда `Text` или `Internal`. Архитектурно подготовлено к остальным. **Modality enum не реализован — отложено до появления VisionPerceptor/AudioPerceptor.**
...
## 2. V7 — Generative Subsystems 🔄 В ПРОЦЕССЕ
...
### 2.2 Когда подсистема становится кандидатом ✅ РЕАЛИЗОВАНО (H1)
...
### 2.3 Жизненный цикл подсистемы ✅ РЕАЛИЗОВАНО (H2)
...
### 2.9 Genome для эмерджентных подсистем ⏳ DEFERRED
...
## 3. V8 — Axiogenesis through Dilemmas ⏳ DEFERRED
...
## 4. V9 — Active NeuralAdvisor Phase ⏳ DEFERRED

### 4.1 Главная идея

Промежуточное состояние — **NeuralAdvisor V3.0** (2026-05-26):
```
V3 NeuralAdvisor (текущий):
  depth: ReactivationDepthAdvisor                 (правила)
  octant: DepthHistoryBiasAdvisor                 (правила + история)
  conflict: PatternLearningResolver               (G2: online learning на AdvisoryHistory)
  subsystem: AnchorVotingAdvisor                  (якорное голосование)
  emergent: DepthThresholdEmergentDetector        (пороговый детектор)
  + G1: DivergenceLog (ring 256)
  + G3: NeuralAdvisorConfig из genome.yaml
```

Финальная цель — **V9 NeuralAdvisor** — все пять трейтов с **реальными обученными моделями**:

```
V9 NeuralAdvisor (plan):
