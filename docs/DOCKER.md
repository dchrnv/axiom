# Axiom OS - Docker Deployment Guide
...
```bash
docker build -t axiom-os:v0.44.0 .
```
...
docker run -d \
  ...
  -p 8080:8080 \
  -e RUST_LOG=debug \
  -v axiom-data:/app/data \
  -v axiom-logs:/app/logs \
  axiom-os:v0.43.0
```
...
- **axiom-data:** WAL files, snapshots
...
```bash
docker exec axiom-api ls -la /app/data/axiom_crash_dump_*.json
```
  ...
  build:
    ...
    steps:
      ...
      - name: Build Docker image
        run: docker build -t axiom-os:${{ github.sha }} .
      - name: Test container
        run: |
          docker run -d --name test axiom-os:${{ github.sha }}
          sleep 5
...
```bash
docker tag axiom-os:v0.43.0 youruser/axiom-os:v0.43.0
docker push youruser/axiom-os:v0.43.0
```
...
- `axiom_tokens_created_total` - Token creation rate
- `axiom_memory_used_bytes` - Memory usage
- `axiom_token_creation_duration_seconds` - Performance
- `axiom_panics_recovered_total` - Reliability
