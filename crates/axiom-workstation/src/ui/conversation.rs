...
fn message_feed<'a>(messages: &'a [ConversationMessage]) -> Element<'a, Message> {
    if messages.is_empty() {
        return container(
            text("No messages yet. Submit text to start.").size(13)
...
fn message_card<'a>(msg: &'a ConversationMessage) -> Element<'a, Message> {
    match msg {
        ConversationMessage::User { text: txt, target_domain, timestamp_secs } => {
            ...
            column![
                row![
                    ...
                    horizontal_space(),
                    text(domain_label).size(11).color(Color::from_rgb(0.5, 0.5, 0.5)),
                ],
        ...
        ConversationMessage::System { text: txt, timestamp_secs, kind } => {
...
fn input_panel<'a>(conv: &'a ConversationState) -> Element<'a, Message> {
    ...
    let submit_btn = button(text(submit_label).size(13))
        .on_press_maybe(if can_submit { Some(Message::ConversationSubmit) } else { None })
        .style(button::primary);

    column![
        text("Target domain:").size(11).color(Color::from_rgb(0.5, 0.5, 0.5)),
        row(domain_btns).spacing(4),
        row![
            text_input("Enter text and press Enter or Submit...", &conv.input_buffer)
