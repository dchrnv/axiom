...
Примеры использования WebSocket возможностей Axiom (v0.60.0 и v0.60.1).
...
```python
from axiom_ws_client import NeurographWSClient, Channel
...
```typescript
import NeurographWSClient from "./axiom-ws-client";
...
```
axiom_ws_connections_total          # Активные соединения
axiom_ws_connections_opened_total   # Всего открыто
axiom_ws_connections_closed_total   # Всего закрыто
axiom_ws_connection_duration_seconds # Длительность
axiom_ws_messages_sent_total        # Отправлено
axiom_ws_messages_received_total    # Получено
axiom_ws_message_size_bytes         # Размер
axiom_ws_message_latency_seconds    # Latency
axiom_ws_subscriptions_total        # Подписки
axiom_ws_channel_subscribers        # Подписчики
axiom_ws_buffered_events            # Буфер
axiom_ws_errors_total               # Ошибки
axiom_ws_broadcast_duration_seconds # Broadcast время
axiom_ws_reconnections_total        # Переподключения
axiom_ws_rate_limit_hits_total      # Rate limit срабатывания
```
