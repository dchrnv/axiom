...
**Текущее состояние:**  
PRIM-TD-03 в ROADMAP.md — это **первый шаг**: реализовать pull/push для val_beneficial/
val_harmful (притяжение/отталкивание) и A5/A6 (тянут Frame-ы из нижних слоёв).

Но за PRIM-TD-03 стоит более широкий принцип, который пока не реализован нигде:
любой STATE_LOCKED якорь с достаточной массой является потенциальным аттрактором —
не только ценности и абстракции.

**Якоря STATE_LOCKED → позиции фиксированы навечно** — можно кешировать безопасно,
механизм обновления не нужен.

**Ключевые id и позиции (сверено с yaml):**
```
val_beneficial       [8000, 12000, 13000]   — притяжение
val_harmful          [3000,  1000, 11000]   — отталкивание
abstraction_theory   [13000, 10000, 14000]  — C5/A5, тянет вглубь
abstraction_constructor [14000, 12000, 15000] — C5+/A6
```

**Когда:** после PRIM-TD-03 (который закрывает первый конкретный случай); полная
обобщённая семантическая гравитация — после накопления понимания как якоря
взаимодействуют на реальных данных.
