...
### AE-TD-08 — Полноценное подключение якорей при инъекции текста (замена позиционного фallback)

**Где:** `crates/axiom-agent/src/perceptors/text.rs`, `crates/axiom-runtime/src/over_domain/axial_evaluator/mod.rs`

Текущий фикс AE-TD-07 (позиционный фallback когда `participants < 2`) — прагматически верен, но архитектурно неполон. Правильное решение: при инъекции текста TextPerceptor должен не только вычислять позицию, но и создавать рёбра связей (ConnectToken команды) от нового токена к matched anchor-токенам. Тогда participants будут непустыми, и стандартный расчёт метрик (entropy/density/will) заработает по-настоящему.

**Почему сейчас фallback корректен:** позиция токена вычислена из семантических якорей (TextPerceptor) и несёт смысловой контекст. Октантное распределение в OBS-03c подтвердило корректность.

**Почему нужен полный фикс:** с реальными связями появится:
- ненулевой entropy (разброс позиций участников) — реальная X-ось
- ненулевой density (граф связей) — реальная Y-ось
- ненулевой will (энергетика связанных токенов) — реальная Z-ось
- более точные ConflictDiagnosis advisories

**Когда:** после базовой стабилизации; приоритет средний — фallback работает достаточно хорошо.
...
### AE-TD-07 — Маршрутизация AE концентрирует всё в один октант ✅ ЗАКРЫТО

**Где:** `crates/axiom-runtime/src/over_domain/axial_evaluator/`

OBS-03 (50k тиков, 19-entry корпус): O7 avg depth = 2196, все остальные октанты O1-O6, O8 = 0. Per-text accuracy 56.2% — AE маршрутизирует большинство инъекций в доминирующий октант (index 6) вместо распределения по всем октантам в соответствии с содержимым текста.

**Симптомы:** dominant octant = 6 с тика ~500 и никогда не меняется; avg_depths[0..5] и avg_depths[7] = 0 несмотря на разнообразный корпус; `writing_narrative` детектируется как `logic`, `heavy_meta_reflection` как `writing` и т.п.

**Гипотезы:** (а) октантные якоря не загружены / не матчатся → всё падает в дефолтный октант; (б) depth accumulation считается только для dominant octant фрейма, не для всех активных; (в) AnchorMatchTable не покрывает октантный маппинг.

**Связь:** Anchor-id задача ниже — вероятно та же корневая причина.

**Когда:** перед следующим OBS-прогоном; без этого фикса multi-octant emergent detection невозможен.
