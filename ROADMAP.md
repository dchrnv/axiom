...
### ~~PRIM-TD-03 — ValueGravity и AbstractionGravity~~ ✅ ГОТОВО (2026-06-04)
...
3. **Интегрировать в `apply_gravity_batch`** или новую функцию `apply_subsystem_gravity`:
   ...
   val или отдельный интервал): применять правила к токенам нужных доменов
   Не нарушать инвариант: wall-clock запрещён, только event_id
   Хранение правил в AxiomEngine (Vec`<SubsystemGravityRule>` — вычисляется при boot, не меняется).
