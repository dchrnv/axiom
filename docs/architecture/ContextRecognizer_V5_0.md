...
## 12. V1 — известные ограничения (→ V2)
...
после каждого цикла `AxialEvaluator`. Механизма межкомпонентной коммуникации внутри
`on_tick` нет — компоненты получают только `&AshtiCore`.

**Следствие:** если координатор не вызывает `sync_axial_store`, снапшот остаётся пустым.
`current_active_octants_for` возвращает `[]` → план всегда падает в поверхностный регион
`CreativeAffirmation`. Работает, но слепо.

**V2:** добавить механизм event/bus между over-domain компонентами, либо shared-state
через `Arc<RwLock<AxialStore>>`.

### CR-TD-02 — Первые тики «слепые» без данных AxialEvaluator

Даже при корректной синхронизации первые несколько тиков `AxialEvaluator` ещё не успел
накопить оценки. `ScanningPlan` будет использовать поверхностный fallback-регион
`CreativeAffirmation` вместо реальных активных октантов.

**Следствие:** подсистема детектируется по энергии MAYA, но без октантного контекста.
Профили создаются с `primary_octant = CreativeAffirmation` пока AxialStore не заполнен.

**V2:** можно сделать тёплый старт — при первом тике опросить все октанты через
`all_octants_in_store`, а не только "активные в окне".

### CR-TD-03 — subsystem_refs должны быть загружены до первого тика

`energy::compute_energies` возвращает пустой результат если `subsystem_refs` пуст.
`ContextRecognizer::new(HashMap::new())` полностью вырожден — работает как no-op.

**Следствие:** всё что нужно для работы — `subsystem_refs` с позициями примитивов
из `config/anchors/*/primitives.yaml`. Если не загружены при инициализации — система
молча не распознаёт подсистемы.

**V2:** добавить `from_anchor_set(AnchorSet)` конструктор, который заполняет refs
автоматически из загруженной `AnchorSet`.
