...
"""
Axiom REST API - FastAPI Application
...
# Force src/python into path to find core axiom package (Rust bindings)
# This must be done before any imports that might trigger 'import axiom'
src_python = Path(__file__).parent.parent / "python"
...
from .exceptions import AxiomException
from .error_handlers import axiom_exception_handler, validation_exception_handler
...
app = FastAPI(
    title="Axiom API",
    description="REST API for Axiom semantic knowledge system",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
...
app.add_exception_handler(AxiomException, axiom_exception_handler)  # type: ignore[arg-type]
...
app.add_middleware(InputSanitizationMiddleware, enable_strict_validation=settings.DEBUG)
...
async def startup_event():
    ...
    """
    logger.info("Axiom API starting up...")
    logger.info(f"Environment: {settings.ENVIRONMENT}")
    logger.info(f"CORS origins: {settings.CORS_ORIGINS}")
    # TODO: Initialize axiom runtime here
