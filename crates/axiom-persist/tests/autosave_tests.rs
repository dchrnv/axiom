...
use axiom_persist::{AutoSaver, PersistenceConfig, load, PersistError};
...
fn test_autosave_disabled_never_saves() {
    ...
    for _ in 0..5 {
        assert!(!saver.tick(&engine), "disabled autosaver must never trigger");
    }
...
fn test_autosave_before_interval() {
    ...
    let cfg = PersistenceConfig::new(&dir, 100);
    let mut saver = AutoSaver::new(cfg);
    let mut engine = AxiomEngine::new();
...
fn test_autosave_respects_interval_after_save() {
    ...
    tick_engine(&mut engine, 100);
    saver.tick(&engine); // первое сохранение
...
fn test_autosave_count() {
    ...
    let cfg = PersistenceConfig::new(&dir, 10);
    let mut saver = AutoSaver::new(cfg);
    let mut engine = AxiomEngine::new();
...
fn test_autosave_file_created_automatically() {
    ...
    let cfg = PersistenceConfig::new(&dir, 50);
    let mut saver = AutoSaver::new(cfg);
    let mut engine = AxiomEngine::new();
...
fn test_autosave_then_load_restores() {
    ...
    let cfg = PersistenceConfig::new(&dir, 30);
    let mut saver = AutoSaver::new(cfg);
    let mut engine = AxiomEngine::new();
...
//
// Симулируем: есть valid manifest + старый engine_state, затем пишем
// engine_state.json.tmp (прерванная запись). Manifest остаётся старым.
// Ожидание: load успешно читает старое состояние.
...
fn test_interrupted_write_old_state_survives() {
    ...
    let cfg = PersistenceConfig::new(&dir, 5);
    let mut saver = AutoSaver::new(cfg);
    saver.tick(&engine);

    let tick_before = engine.tick_count;

    // Симулируем прерванную запись: оставляем .tmp файл
    let tmp_path = dir.join("engine_state.json.tmp");
    std::fs::write(&tmp_path, b"garbage corrupted data").unwrap();
    // Manifest и engine_state.json не трогаем

    // Load должен работать — читает старый engine_state.json, игнорирует .tmp
...
fn test_force_save() {
    ...
    let cfg = PersistenceConfig::new(&dir, 10000); // большой интервал
    let mut saver = AutoSaver::new(cfg);
    let mut engine = AxiomEngine::new();

    tick_engine(&mut engine, 5); // мало тиков, автосохранение не сработает

    assert!(!saver.should_save(&engine));
    saver.force_save(&engine).expect("force_save должен работать");
    assert!(dir.join("manifest.yaml").exists());
