// Axiom - Observability Stack Stress Test
...
const WAL_PATH: &str = "/tmp/axiom_stress_test.wal";
...
fn test_panic_recovery() {
    ...
    let wal_crash_path = "/tmp/axiom_crash_test.wal";
        ...
        let entries_replayed = reader.replay(|entry| {
            match entry.header.entry_type {
                WalEntryType::TokenCreated => {
                    // Verify checksum
                    if !entry.verify() {
                        return Err(axiom_core::wal::WalError::ChecksumMismatch);
                    }

                    // Deserialize token
                    let token_bytes: [u8; 64] = entry.payload[0..64]
                        .try_into()
                        .map_err(|_| axiom_core::wal::WalError::CorruptedFile)?;
...
fn sanity_check_observability_overhead() {
    ...
    // With WAL
    let wal_path = "/tmp/axiom_sanity.wal";
