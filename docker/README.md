...
Complete Docker setup for Axiom with all services and examples.

## Services

- **axiom-api**: Main Axiom API server
...
docker-compose up -d axiom-api postgres redis
...
docker-compose build axiom-api
```
...
docker-compose exec axiom-api python -c "print('hello')"

# Open shell in API container
docker-compose exec axiom-api sh

# Run tests
docker-compose exec axiom-api pytest
```
...
```bash
docker-compose exec axiom-api alembic upgrade head
```
...
```bash
docker-compose exec postgres psql -U axiom -d axiom
```
...
# Verify network
docker network ls
docker network inspect docker_axiom-network
...
# Test connection
docker-compose exec axiom-api nc -zv postgres 5432
```
...
# Tag
docker tag axiom/api:latest axiom/api:0.59.3

# Push
docker push axiom/api:latest
docker push axiom/api:0.59.3
```
